#!/usr/bin/env bash
# Assemble android/app/src/main/jniLibs/<abi>/ from pre-built
# android/native/ out/ trees, mirroring android/native/install-to-app.sh
# but parameterized so CI can run it against artifacts downloaded from a
# prior workflow job.
#
# Usage:
#   ./scripts/install-natives-android.sh <abi> <path-to-native-out-root>
#
# where <path-to-native-out-root> contains bin/ and lib/ (the install
# prefix produced by android/native/build.sh).
set -euo pipefail
